...
// .wrangler/tmp/pages-2SQfty/functionsWorker-0.546459154961204.mjs
...
var keys = /* @__PURE__ */ __name2((raw) => (raw ?? "").split(",").map((k) => k.trim()).filter(Boolean), "keys");
async function callGemini(prompt, apiKeys, wantJson, model, maxTokens = 1024) {
  let currentKeyIndex = 0;
  let lastErr = null;
  const attempt = /* @__PURE__ */ __name2(async (apiKey) => {
...
__name2(buildRecommendPrompt, "buildRecommendPrompt");
function buildReplyPrompt(userData, message) {
  ...
  return [
    ...
    `Responde como Auki con un mensaje corto (3-4 frases), cercano, motivador y entusiasta, reaccionando a esa respuesta.`,
    `M\xE1ximo 350 caracteres. Termina siempre con un punto final.`,
    `No uses emojis. No hagas preguntas nuevas, no repitas literalmente sus palabras y no menciones pilares todav\xEDa.`
  ].join("\n");
}
__name(buildReplyPrompt, "buildReplyPrompt");
__name2(buildReplyPrompt, "buildReplyPrompt");
...
__name2(matchPilar, "matchPilar");
async function recommend(userData, apiKeys, model) {
  const text = await callGemini(buildRecommendPrompt(userData), apiKeys, true, model, 2048);
  const parsed = parseJson(text) ?? {};
  const pilar = matchPilar(String(parsed.pilar ?? ""));
  const nombre = userData.nombre.trim() || "estudiante";
  const str = /* @__PURE__ */ __name2((v, fallback) => typeof v === "string" && v.trim() ? v.trim() : fallback, "str");
  const arr = /* @__PURE__ */ __name2((v, fallback) => Array.isArray(v) && v.length > 0 ? v.map(String) : fallback, "arr");
  return {
    nombre,
    pilarId: pilar.id,
    pilar: pilar.nombre,
...
// .wrangler/tmp/bundle-I5wncL/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-I5wncL/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.546459154961204.js.map
